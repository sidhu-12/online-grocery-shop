...
export default class Cart extends Component{
   constructor(props)
   {
       super(props);
       this.state={
        op:this.props.route.params.cart,
        key:-1,
        visible:false,
...
else
{
  Alert.alert("Selected product is not added to cart already");
    ...
    <Text style={{textAlign:"center",fontWeight:'bold',fontSize:30}}>Total:{this.props.route.params.total}</Text>
        <Button title="Order"
        onPress={()=>this.props.navigation.navigate("Confirmation",{cart:this.cart,total:this.total})}/>
        </View>
